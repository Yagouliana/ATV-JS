...
// ================= ALTERNAÇÃO DE MODO CLARO/ESCURO =================
...
function toggleMode() {
    ...
    // Atualiza a cor do texto do título (caso necessário)
    updateTextColor();
}
...
let titleElement = document.querySelector('#name');
let text = "YAGO";
let index = 0;
let isTyping = true;
...
const carouselsSlides = document.querySelector('.carousel-slides');
...
function updateSlidePosition() {
    ...
    carouselsSlides.style.transform = `translateX(-${currentSlide * slideWidth}px)`;
}

// Função para avançar para o próximo slide
function nextSlide() {
    showSlide(currentSlide + 1);
    restartAutoSlide(); // Reinicia o intervalo de transição automática
}
...
carouselsSlides.parentElement.addEventListener('mouseenter', () => {
    clearInterval(autoSlideInterval);
});

// Retorna a transição automática ao remover o mouse do carrossel
carouselsSlides.parentElement.addEventListener('mouseleave', startAutoSlide);
